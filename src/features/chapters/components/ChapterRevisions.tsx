...
export function ChapterRevisions({ revisions, onToggle }: ChapterRevisionsProps) {
  const [isExpanded, setIsExpanded] = useLocalStorage('os12-revisions-expanded', true)
  ...
  const completionPercent = Math.round((completedCount / REVISION_DAYS.length) * 100)
    ...
    <Card className="overflow-hidden border-border/50 bg-card/50 transition-all duration-300">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-semibold text-sm text-foreground">Spaced Repetition</span>
        </div>
          ...
          <CardContent className="p-3 border-t border-border/50">
            <div className="flex flex-col gap-2 relative">
              ...
              {REVISION_DAYS.map(day => {
                ...
                return (
                  <div key={day} className="flex items-center justify-between p-2.5 rounded-xl border bg-background hover:border-primary/50 transition-colors relative z-10 group">
                    <div className="flex items-center gap-3">
                      <div className={cn("h-6 w-6 rounded-full flex items-center justify-center shrink-0 border-2 transition-colors",
                      ...
                      <div className="flex flex-col">
                        <span className="font-semibold text-[13px]">Day {day} Revision</span>
                        <span className="text-[9px] uppercase font-bold tracking-wider text-muted-foreground">Review materials</span>
                      </div>
                    </div>
                    <button
                      onClick={() => onToggle(day, nextStatus)}
                      className={cn("flex items-center gap-2 px-3 py-1.5 rounded-full text-[10px] uppercase font-bold tracking-wider transition-all",
                        isCompleted 
                          ? 'bg-green-500/10 text-green-600 hover:bg-green-500/20' 
                          : 'bg-muted text-muted-foreground opacity-0 group-hover:opacity-100 hover:bg-primary/10 hover:text-primary focus:opacity-100'
                      )}
                    >
